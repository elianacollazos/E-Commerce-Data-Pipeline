...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    plt.plot(df["date"], df["order_count"], label="Order Count per Day")

    holidays = df[df["holiday"] == True]["date"]
    for holiday in holidays:
        plt.axvline(x=holiday, color="red", linestyle="--", alpha=0.5, label="Holiday" if "Holiday" not in plt.gca().get_legend_handles_labels()[1] else "")
