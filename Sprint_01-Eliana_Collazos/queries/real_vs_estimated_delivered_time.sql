...
avg_times_by_month AS (
    SELECT
        month_no,
        ROUND(AVG(CASE WHEN year = '2016' THEN real_time END), 2) AS Year2016_real_time,
        ROUND(AVG(CASE WHEN year = '2017' THEN real_time END), 2) AS Year2017_real_time,
        ROUND(AVG(CASE WHEN year = '2018' THEN real_time END), 2) AS Year2018_real_time,
        ROUND(AVG(CASE WHEN year = '2016' THEN estimated_time END), 2) AS Year2016_estimated_time,
        ROUND(AVG(CASE WHEN year = '2017' THEN estimated_time END), 2) AS Year2017_estimated_time,
        ROUND(AVG(CASE WHEN year = '2018' THEN estimated_time END), 2) AS Year2018_estimated_time
    FROM orders_with_durations
    GROUP BY month_no
)
SELECT
    month_no,
